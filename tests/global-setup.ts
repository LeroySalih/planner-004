...
export default async function globalSetup() {
  console.log("[global-setup] Checking if server is already running...");
  let serverAlreadyRunning = false;
  try {
    if (await waitForServer(BASE_URL, 2000)) {
      serverAlreadyRunning = true;
      console.log("[global-setup] Server is already running.");
    }
  } catch {
    console.log("[global-setup] Server not running, starting it...");
  }
  ...
  console.log(
    "[global-setup] Server is up, creating authenticated storage state...",
  );
  ...
  await page.getByRole("textbox", { name: "Email address" }).fill(
    process.env.PW_EMAIL ?? DEFAULT_EMAIL,
  );
  await page.getByRole("textbox", { name: "Password" }).fill(
    process.env.PW_PASSWORD ?? DEFAULT_PASSWORD,
  );
  ...
  if (child && child.pid) {
    console.log(
      `[global-setup] Server running with pid ${child.pid}. It will be stopped in global-teardown.`,
    );
  }
}
