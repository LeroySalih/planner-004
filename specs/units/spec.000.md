...
- Lesson creation now uses the async job pattern. `triggerLessonCreateJobAction` queues the insert, broadcasts a single `lesson created` event on `lesson_updates`, and the Lessons panel swaps pending placeholders with the realtime payload.
