...
3. Introduce derived calculation tables dedicated to powering `/reports`, aggregated per student so `/reports/{pupilId}` can render directly from a single row while `/reports/groups/{groupId}` derives cohort summaries from those student-level aggregates. Provide a migration-time backfill that seeds these tables from existing feedback scores so the prototype works immediately on current data.
