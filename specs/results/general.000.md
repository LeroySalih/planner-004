...
- AI feedback arriving via `/webhooks/ai-mark` dispatches Supabase Realtime events on the same assignment channel (`results:assignments:{group__lesson}`) with `{ submissionId, pupilId, activityId, aiScore, aiFeedback, successCriteriaScores }`, where each success criterion receives the same `aiScore` value. Automatic Score panes re-render in place while the rest of the matrix stays interactive.
