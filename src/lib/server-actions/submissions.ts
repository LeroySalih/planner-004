...
export async function getLatestSubmissionForActivityAction(
  ...
  const input = McqSubmissionInputSchema.pick({
    activityId: true,
    userId: true,
  }).parse({
    activityId,
  ...
  if (!parsed.success) {
    console.error(
      "[submissions] Failed to parse submission row:",
      parsed.error,
    );
...
export async function readLessonSubmissionSummariesAction(
...
): Promise<
  {
    ...
    averages: LessonAverageBreakdown;
    error: string | null;
  }
> {
  ...
  try {
    ...
    try {
      ...
      console.error(
        "[submissions] Failed to read lesson activities for feedback summary:",
        activitiesError,
      );
      return {
        data: [],
        averages: { activitiesAverage: null, assessmentAverage: null },
        error: activitiesError instanceof Error
          ? activitiesError.message
          : "Unable to load activities.",
      };
    }

    const activities = LessonActivitySummaryRowSchema.array().parse(
      activityRows ?? [],
    );
    const scorableActivities = activities.filter((activity) =>
    ...
    try {
      ...
      console.error(
        "[submissions] Failed to read lesson submissions for feedback summary:",
        submissionsError,
      );
      return {
        data: [],
        averages: { activitiesAverage: null, assessmentAverage: null },
        error: submissionsError instanceof Error
    ...
    for (const activity of scorableActivities) {
      ...
      const activityTitle = (activity.title ?? "Untitled activity").trim() ||
        "Untitled activity";
      ...
      if (activityType === "multiple-choice-question") {
        const parsedActivity = McqActivityBodySchema.safeParse(
          activity.body_data,
        );
        const mcqOptions = parsedActivity.success
          ? parsedActivity.data.options
          : [];
        const correctOptionId = parsedActivity.success
          ? parsedActivity.data.correctOptionId
          : null;
        ...
        const scoreEntries = submissionList
          .map((submission) => {
            const parsedSubmission = McqSubmissionBodySchema.safeParse(
              submission.body,
            );
        ...
        const generalScores = submissionList.map((submission) => {
          ...
          if (body && typeof body === "object") {
            ...
            if (rawSuccessCriteria && typeof rawSuccessCriteria === "object") {
              successCriteriaScores = rawSuccessCriteria as Record<
                string,
                number | null
              >;
            }
          ...
          const averagedScore = computeAverageSuccessCriteriaScore(
            normalisedScores,
          );
          const finalScore = averagedScore ?? overrideScore ?? baseScore ??
            null;
    ...
    console.error(
      "[submissions] Unexpected error building submission summaries:",
      error,
    );
    return {
      data: [],
...
export async function upsertMcqSubmissionAction(
  ...
  try {
    ...
    console.error(
      "[submissions] Failed to load activity for submission:",
      error,
    );
    return {
      success: false,
  ...
  if (!parsedActivity.success) {
    console.error(
      "[submissions] Invalid MCQ activity body:",
      parsedActivity.error,
    );
    return {
      success: false,
      error: "Question is not configured correctly.",
      data: null as Submission | null,
    };
  }
  ...
  const optionExists = mcqBody.options.some((option) =>
...
export async function readSubmissionByIdAction(submissionId: string) {
  try {
    const { rows } = await query(
    ...
    const parsed = SubmissionSchema.safeParse(row);
    if (!parsed.success) {
      console.error("[submissions] Failed to parse submission:", parsed.error);
      return { success: false, error: "Invalid submission data.", data: null };
    }

    return { success: true, error: null, data: parsed.data };
  } catch (error) {
    console.error("[submissions] Failed to read submission by ID:", error);
    const message = error instanceof Error
      ? error.message
      : "Unable to load submission.";
