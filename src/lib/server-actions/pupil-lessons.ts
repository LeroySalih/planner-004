...
export type PupilLessonsSummaryBootstrap = z.infer<
  typeof PupilLessonsSummaryBootstrapSchema
>;
export type PupilLessonsDetailBootstrap = z.infer<
  typeof PupilLessonsDetailBootstrapSchema
>;
  ...
  return withTelemetry(
    ...
    async () => {
      try {
        ...
        const payloadValue = (rawRow as Record<string, unknown> & {
          pupil_lessons_summary_bootstrap?: unknown;
        })
          ?.pupil_lessons_summary_bootstrap ??
          (rawRow as Record<string, unknown> & {
            pupilLessonsSummaryBootstrap?: unknown;
          })
        ...
        const lessonAssignments = Array.isArray(
            (payload as Record<string, unknown>)?.lesson_assignments,
          )
          ? ((payload as { lesson_assignments: unknown[] })
            .lesson_assignments as any[])
          : (payload as Record<string, unknown> & {
            lessonAssignments?: unknown[];
          }).lessonAssignments ?? [];

        const visibleLessonAssignments = (lessonAssignments as any[]).filter(
          (assignment) => !assignment.hidden,
        );

        const normalized = {
          pupils: payload.pupils ?? [],
          memberships: payload.memberships ?? [],
          lessonAssignments: visibleLessonAssignments,
        };

        const parsed = PupilLessonsSummaryBootstrapSchema.safeParse(normalized);
        if (!parsed.success) {
          console.error(
            "[pupil-lessons] invalid summary bootstrap payload",
            parsed.error,
          );
          return PupilLessonsSummaryBootstrapReturnSchema.parse({
            data: null,
        ...
        const message = error instanceof Error
          ? error.message
          : "Unable to load pupil lesson summaries.";
        return PupilLessonsSummaryBootstrapReturnSchema.parse({
          data: null,
          error: message,
        });
    ...
    async () => {
      try {
        const { rows } = await query<{
          ...
          learning_objectives:
            PupilLessonsDetailBootstrap["learningObjectives"];
          success_criteria: PupilLessonsDetailBootstrap["successCriteria"];
          success_criteria_units:
            PupilLessonsDetailBootstrap["successCriteriaUnits"];
        }>(
          `
            select *
        ...
        const payloadValue = (rawRow as Record<string, unknown> & {
          pupil_lessons_detail_bootstrap?: unknown;
        })
          ?.pupil_lessons_detail_bootstrap ??
          (rawRow as Record<string, unknown> & {
            pupilLessonsDetailBootstrap?: unknown;
          })
        ...
        const learningObjectives =
          Array.isArray((payloadRecord as any)?.learning_objectives)
            ? ((payload as { learning_objectives: unknown[] })
              .learning_objectives as unknown[])
            : (payloadRecord as Record<string, unknown> & {
              learningObjectives?: unknown[];
            }).learningObjectives ?? [];
        const successCriteria =
          Array.isArray((payloadRecord as any)?.success_criteria)
            ? ((payload as { success_criteria: unknown[] })
        ...
        const successCriteriaUnits =
          Array.isArray((payloadRecord as any)?.success_criteria_units)
            ? ((payload as { success_criteria_units: unknown[] })
              .success_criteria_units as unknown[])
            : (payloadRecord as Record<string, unknown> & {
              successCriteriaUnits?: unknown[];
        ...
        if (!parsed.success) {
          console.error(
            "[pupil-lessons] invalid detail bootstrap payload",
            parsed.error,
          );
          return PupilLessonsDetailBootstrapReturnSchema.parse({
            data: null,
        ...
        console.error("[pupil-lessons] detail bootstrap query failed", {
          pupilId,
          error,
        });
        const message = error instanceof Error
          ? error.message
          : "Unable to load pupil lesson detail.";
        return PupilLessonsDetailBootstrapReturnSchema.parse({
          data: null,
          error: message,
        });
