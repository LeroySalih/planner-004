...
export type LessonHeaderUpdateState = z.infer<
...
async function runLessonCreateJob(
  { jobId, unitId, title }: LessonCreateJobArgs,
) {
  try {
    ...
    const message = error && typeof error === "object" && "message" in error
      ? String(
        (error as { message?: string }).message ?? "Failed to create lesson",
      )
      : "Failed to create lesson";

    console.error("[lessons] async create lesson job failed", {
      unitId,
      jobId,
      error,
    });
    ...
    try {
      ...
      console.error("[lessons] failed to publish lesson job error", {
        jobId,
        notifyError,
      });
    }
...
export async function readLessonsByUnitAction(
  ...
  return withTelemetry(
    ...
    async () => {
      ...
      const { lessons: enrichedLessons, error: scError } =
        await enrichLessonsWithSuccessCriteria(lessons);

      if (scError) {
        console.error(
          "[v0] Failed to read success criteria for lessons:",
          scError,
        );
      ...
      const normalized = enrichedLessons.map((lesson) => {
        ...
        const filtered =
          ((lessons_learning_objective ?? []) as LessonLearningObjective[])
...
export async function readLessonsAction(
  ...
  return withTelemetry(
    ...
    async () => {
      ...
      if (scError) {
        console.error(
          "[v0] Failed to read success criteria for all lessons:",
          scError,
        );
        return LessonsReturnValue.parse({ data: null, error: scError });
      }
...
export async function createLessonAction(
  ...
  console.log("[v0] Server action started for lesson creation:", {
    unitId,
    title,
  });
  ...
  if (!createdLessonId) {
    return LessonReturnValue.parse({
      data: null,
...
export async function updateLessonAction(
  ...
  console.log("[v0] Server action started for lesson update:", {
    lessonId,
    unitId,
    title,
  });
  ...
  try {
    await withDbClient(async (client) => {
      await client.query("update lessons set title = $1 where lesson_id = $2", [
        title,
        lessonId,
      ]);
      ...
      const existingIds = new Set(
        (existingLinks ?? []).map((link) => link.learning_objective_id),
      );
      ...
      const idsToInsert = sanitizedObjectiveIds.filter((id) =>
        !existingIds.has(id)
      );

      for (
        const [index, learningObjectiveId] of sanitizedObjectiveIds.entries()
      ) {
      ...
      if (idsToInsert.length > 0) {
        ...
        idsToInsert.forEach((learningObjectiveId) => {
          values.push(
            learningObjectiveId,
            lessonId,
            sanitizedObjectiveIds.indexOf(learningObjectiveId),
            title,
          );
          placeholders.push(
            `($${values.length - 3}, $${values.length - 2}, $${
...
async function applyLessonSuccessCriteriaUpdate(
  ...
  await withDbClient(async (client) => {
    ...
    const idsToInsert = normalizedIds.filter((id) =>
      !existingCriteriaIds.has(id)
    );
    const idsToDelete = Array.from(existingCriteriaIds).filter((id) =>
    ...
    for (const row of existingObjectiveLinks ?? []) {
      ...
      existingObjectiveOrderMap.set(
        ...
        typeof row.order_by === "number"
          ? row.order_by
          : existingObjectiveOrderMap.size,
      );
    }
    ...
    if (objectiveIdsToInsert.length > 0) {
      ...
      const maxExistingOrder = existingOrders.length > 0
        ? Math.max(...existingOrders)
        : -1;

      const insertRows = (learningObjectiveMetadata ?? []).map((
        meta,
        index,
      ) => ({
      ...
      if (insertRows.length > 0) {
        ...
        insertRows.forEach((row) => {
          ...
          placeholders.push(
            `($${values.length - 4}, $${values.length - 3}, $${
              values.length - 2
    ...
    async () => {
      ...
      try {
        await withDbClient(async (client) => {
          ...
          nextOrder =
            typeof maxOrderRow?.[0]?.order_index === "number" &&
              Number.isFinite(maxOrderRow[0].order_index)
          ...
          const { successCriterionDescription, successCriterionLevel } =
            payload;
          ...
          createdCriterion = {
            ...
            description: insertedCriterion.description ??
              successCriterionDescription,
          ...
          const nextLessonObjectiveOrder =
            (lessonObjectiveOrders ?? []).reduce<number>((max, row) => {
              const value = typeof row?.order_by === "number"
                ? row.order_by
                : -1;
        ...
        console.error(
          "[lessons] Failed to create learning objective via PG:",
          error,
        );
        const message = error instanceof Error
          ? error.message
      ...
      const normalizedObjective = {
        ...
        assessment_objective_id:
          assessmentObjectiveRecord.assessment_objective_id ??
            objectiveRecord.assessment_objective_id,
        ...
        assessment_objective_order_index:
          assessmentObjectiveRecord.order_index ?? null,
        assessment_objective_curriculum_id:
          assessmentObjectiveRecord.curriculum_id ?? null,
...
export async function createLessonLearningObjectiveFormAction(
  ...
  try {
    ...
    const assessmentObjectiveId = String(
      formData.get("assessmentObjectiveId") ?? "",
    ).trim();
    ...
    const specRef =
      typeof specRefEntry === "string" && specRefEntry.trim().length > 0
        ? specRefEntry
        : null;
    const successCriterionDescription = String(
      formData.get("successCriterionDescription") ?? "",
    ).trim();
    const successCriterionLevelValue = formData.get("successCriterionLevel");
    const successCriterionLevel = typeof successCriterionLevelValue === "string"
      ? Number.parseInt(successCriterionLevelValue, 10)
      : Number.NaN;

    if (
      !lessonId || !assessmentObjectiveId || !title ||
      successCriterionDescription.length === 0
    ) {
      return {
        status: "error",
        message:
          "Lesson, assessment objective, title, and success criterion are required.",
        learningObjective: null,
      };
    }

    if (
      !Number.isInteger(successCriterionLevel) || successCriterionLevel < 1 ||
      successCriterionLevel > 9
    ) {
      return {
        status: "error",
    ...
    console.error(
      "[lessons] Failed to create learning objective via form:",
      error,
    );
    return {
      status: "error",
  ...
  return withTelemetry(
    ...
    async () => {
      ...
      try {
        ...
        console.error(
          "[lessons] Failed to create success criterion via PG:",
          error,
        );
        const message = error instanceof Error
          ? error.message
          : "Unable to create success criterion.";
      ...
      const learningObjectiveRecord = learningObjective as
        ...
        | null;
      const rawAssessmentObjective =
        learningObjectiveRecord?.assessment_objective ?? null;
...
export async function createLessonSuccessCriterionFormAction(
  ...
  try {
    ...
    console.error(
      "[lessons] Failed to create success criterion via form:",
      error,
    );
    return {
      status: "error",
...
export async function deactivateLessonAction(lessonId: string, unitId: string) {
  console.log("[v0] Server action started for lesson deactivation:", {
    lessonId,
    unitId,
  });

  try {
    await query("update lessons set active = false where lesson_id = $1", [
      lessonId,
    ]);
  } catch (error) {
...
async function enrichLessonsWithSuccessCriteria<
  T extends {
    lesson_id?: string;
    lessons_learning_objective?: LessonLearningObjective[];
  },
>(lessons: T[]): Promise<{ lessons: T[]; error: string | null }> {
      ...
      for (const row of activityRows ?? []) {
        ...
        const activityType =
          typeof (row as { type?: unknown }).type === "string"
            ...
            : null;
        activitySummativeFlags.set(
          activityId,
          rawIsSummative && isScorableActivityType(activityType),
        );
      }
    } catch (activitiesError) {
      console.error(
        "[lessons] Failed to load activities for enrichment:",
        activitiesError,
      );
  ...
  if (lessonIds.length > 0) {
    try {
      ...
      lessonCriteriaRows = lessonCriteriaRows.map((row) => ({
        ...row,
        activity_id: null,
      }));
    } catch (lessonCriteriaError) {
      console.error(
        "[lessons] Failed to load lesson success criteria:",
        lessonCriteriaError,
      );
    ...
    if (missingIds.length > 0) {
      try {
        ...
        for (const row of missingRows ?? []) {
          const successCriteriaId = typeof row?.success_criteria_id === "string"
          ...
          const learningObjectiveId =
            typeof row.learning_objective_id === "string"
              ? row.learning_objective_id
              : null;
        ...
        console.error(
          "[lessons] Failed to load missing success criteria:",
          missingError,
        );
        return {
          lessons: [],
          error: "Unable to load success criteria metadata.",
        };
      }
    }
  }
  ...
  if (lessonCriteriaRows.length > 0) {
    try {
      const { rows } = await query(
        ...
        [
          Array.from(
            new Set(lessonCriteriaRows.map((row) => row.success_criteria_id)),
          ).filter(
            (id) => Boolean(id),
          ),
        ],
      ...
      console.error(
        "[lessons] Failed to load success criteria metadata:",
  ...
  for (const row of criteriaMetadataRows) {
    ...
    const description = typeof row.description === "string"
      ...
      : null;
    const level = typeof row.level === "number" ? row.level : null;
    const learningObjectiveId = typeof row.learning_objective_id === "string"
      ? row.learning_objective_id
      : null;
    ...
    if (learningObjectiveId) {
      ...
      if (
        !list.some((entry) =>
          entry.success_criteria_id === row.success_criteria_id
        )
      ) {
        list.push({
          success_criteria_id: row.success_criteria_id,
  ...
  const metadataIdsToFetch = Array.from(ids).filter((id) =>
    !learningObjectiveMetadata.has(id)
  );
    ...
    try {
      ...
      for (const row of learningObjectiveRows ?? []) {
        ...
        learningObjectiveMetadata.set(learningObjectiveId, {
          ...
          assessment_objective_id:
            typeof row.assessment_objective_id === "string"
              ? row.assessment_objective_id
              : null,
          assessment_objective_title:
            typeof row.assessment_objective_title === "string"
              ? row.assessment_objective_title
              : null,
          assessment_objective_code:
            typeof row.assessment_objective_code === "string"
      ...
      console.error(
        "[lessons] Failed to load learning objective metadata:",
  ...
  const enriched = lessons.map((lesson) => {
    const updatedObjectives = (lesson.lessons_learning_objective ?? []).map(
      (entry) => {
        const loId = entry.learning_objective_id ??
          entry.learning_objective?.learning_objective_id ?? "";
        ...
        const mergedLearningObjective = loId
          ? {
            ...
            assessment_objective_id: metadata?.assessment_objective_id ??
              entry.learning_objective?.assessment_objective_id ?? null,
            assessment_objective_title: metadata?.assessment_objective_title ??
              entry.learning_objective?.assessment_objective_title ?? null,
            assessment_objective_code: metadata?.assessment_objective_code ??
              entry.learning_objective?.assessment_objective_code ?? null,
            assessment_objective_order_index:
              metadata?.assessment_objective_order_index ??
                entry.learning_objective?.assessment_objective_order_index ??
                null,
            title: metadata?.title ?? entry.learning_objective?.title ??
              entry.title ?? "Learning objective",
            order_index: metadata?.order_index ??
              entry.learning_objective?.order_index ?? entry.order_by ?? 0,
            active: metadata?.active ?? entry.learning_objective?.active ??
              true,
            spec_ref: metadata?.spec_ref ??
              entry.learning_objective?.spec_ref ?? null,
            ...
            assessment_objective: metadata?.assessment_objective_id
              ...
              : entry.learning_objective &&
                  "assessment_objective" in entry.learning_objective
              ? (entry.learning_objective as Record<string, unknown>)
    ...
    const existingObjectiveIds = new Set(
      updatedObjectives
        .map((objective) => {
          const direct =
            (objective as { learning_objective_id?: string | null })
    ...
    const lessonCriteria = (lessonCriteriaMap.get(lesson.lesson_id ?? "") ?? [])
      .map((row) => {
        const details = detailMap.get(row.success_criteria_id) ?? ({
          description: null,
          level: null,
        ...
        const loIdFromCriterion = row.learning_objective_id ??
          criteriaToObjectiveMap.get(row.success_criteria_id) ??
          details.learning_objective_id;
        ...
        return {
          ...
          is_summative: row.activity_id
            ? activitySummativeFlags.get(row.activity_id) ?? false
            : false,
        };
      });
    ...
    const groupedCriteria = lessonCriteria.reduce<
      Map<string, typeof lessonCriteria>
    >((acc, criterion) => {
    ...
    for (const [loId, criteria] of groupedCriteria.entries()) {
      ...
      derivedObjectives.push({
        ...
        learning_objective: {
          ...
          assessment_objective_order_index:
            metadata.assessment_objective_order_index ?? null,
          title: metadata.title ?? "Learning objective",
    ...
    const combinedObjectives = [...updatedObjectives, ...derivedObjectives]
      .sort(
    ...
    return {
      ...
      lessons_learning_objective:
        combinedObjectives as LessonLearningObjective[],
...
async function loadLessonDetailBootstrapPayload(
  ...
  try {
    ...
    const { rows } = await client.query(
      "select lesson_detail_bootstrap($1) as payload",
      [lessonId],
    );
    ...
    if (!parsed.success) {
      ...
      return LessonDetailReturnValue.parse({
        data: null,
        error: "Unable to parse lesson detail payload",
      });
    }
    ...
    console.error(
      "[lessons] Failed to load lesson detail bootstrap via PG:",
      error,
    );
    const message = error instanceof Error
      ? error.message
      : "Unable to load lesson detail.";
...
async function loadLessonReferencePayload(
  ...
  try {
    ...
    await client.connect();
    const { rows } = await client.query(
      "select lesson_reference_bootstrap($1) as payload",
      [lessonId],
    );
    const payload = rows[0]?.payload ?? null;
    ...
    if (!parsed.success) {
      console.error(
        "[lessons] Invalid lesson reference payload:",
        parsed.error,
      );
    ...
    console.error(
      "[lessons] Failed to load lesson reference payload via PG:",
      error,
    );
    const message = error instanceof Error
      ? error.message
      : "Unable to load lesson reference payload.";
  ...
  return withTelemetry(
    {
      ...
      params: {
        ...
        hasTitle: payload.title.length > 0,
      },
      authEndTime,
    },
    async () => {
      ...
      try {
        ...
        console.error(
          "[lessons] Failed to update lesson header via PG:",
          error,
          {
            lessonId: payload.lessonId,
            userId: profile.userId,
      ...
      const refreshed = await loadLessonDetailBootstrapPayload(
        payload.lessonId,
      );
...
async function readLessonWithObjectives(lessonId: string) {
  ...
  const { lessons: enrichedLessons, error: scError } =
    await enrichLessonsWithSuccessCriteria([lesson]);
...
export async function readLessonDetailBootstrapAction(
  ...
  return withTelemetry(
    ...
    async () => {
      console.log(
        "[lessons] Server action started for lesson detail bootstrap:",
        { lessonId },
      );
...
export async function readLessonReferenceDataAction(
  ...
  return withTelemetry(
    ...
    async () => {
      console.log(
        "[lessons] Server action started for lesson reference payload:",
        { lessonId },
      );
...
export async function toggleLessonActiveAction(
  ...
  try {
    await withDbClient(async (client) => {
      await client.query(
        "update lessons set active = $1 where lesson_id = $2",
        [active, lessonId],
      );
    });
  } catch (error) {
    console.error("[lessons] Failed to toggle lesson active status:", error);
    return { success: false, error: "Unable to update lesson status." };
  }
